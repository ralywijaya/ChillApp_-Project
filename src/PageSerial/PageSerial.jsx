      ...
      <div className="serial-play">
        ...
        <div className="rating-serial-page">
          ...
          <div className="rating-number-serial" style={{ fontSize: "1.2rem" }}>
            {Serial?.vote_average ? (
              <p>{Serial.vote_average.toFixed(1)} / 10</p>
            ) : (
        ...
        <div>
          ...
          >{`Jumblah Season : ${Serial.number_of_seasons} Season`}</p>
          <p
            style={{ fontSize: "1.2rem" }}
          >{`Jumblah episode : ${Serial.number_of_episodes} Episode`}</p>
        </div>
        <div>
          <p style={{ fontSize: "1.2rem" }}>
            {Serial.runtime
              ? `${Serial.episode_run_time?.[0]} menit`
        ...
        <div className="detail-seria-page">
          ...
          <div className="genre-serial-page">
            {Genre?.map((i) => (
              <Link
                to={`/menu_genre_serial/${i.id}/${i.name}`}
                key={i.id}
                style={{ fontSize: "1.2rem" }}
              >
                {i.name}
