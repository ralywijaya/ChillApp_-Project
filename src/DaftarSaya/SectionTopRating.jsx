...
import { AmbilDaftarSayaTMDB,LoadingDaftarSaya,ErrorDaftarSaya } from "../SliceRedux/SliceAkun"
...
export default function SectionTopRating({subJudul
    ...
    const { DaftarSayaTMDB,loading,eror,DaftarVersion}=useSelector((state)=>state.Akun)
...
<div ref={scrollref} className="card-film" >
 ...
 <div className="box-film" key={i.id}>
  ...
  alt={i.title||i.name} onClick={()=>{setisEdit(true )
        setFilmAktif(i.id)
    }}/>
        {filmAktif==i.id&&isEdit&& <HoverFilm setisEdit={setisEdit} Judul={i.title||i.name} i={i} GenreFilm={GenreFilm||GenreSerial}/>}
